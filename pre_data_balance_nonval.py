...
def data_partition_random_XGB_nonval(data,cluster,propertion,Sampler):
    """
    propertion: the propertion of train
    """
    np.random.seed(123)
    k = len(cluster)
    f = data.shape[1]
    ...
    train_index = np.random.choice(class_index_dict[1], int(label_n_per_class[1] * propertion), replace=False)
    train_index = list(train_index)
    test_potential_index = list(set(class_index_dict[1]) - set(train_index))
    test_index = np.random.choice(test_potential_index, int(label_n_per_class[1] * (1-propertion)), replace=False)
    ...
    test_false_potential_index = list(set(class_index_dict[0]) - set(train_index_false))
    test_index_false = np.random.choice(test_false_potential_index, len(test_index), replace=False)
